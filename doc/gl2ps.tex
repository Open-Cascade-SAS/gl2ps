...
%
%  $Id: gl2ps.tex,v 1.6 2001-06-11 11:26:14 geuzaine Exp $
% 
...
\author{Christophe Geuzaine}
\date{Version 0.33, 11 June 2001}
...
\begin{description}
\item[\dd{GL2PS_POLYGON_OFFSET_FILL}] Tries to mimic the
...
\item[\dd{GL2PS_LINE_STIPPLE}] Tries to mimic the
\dd{GL_LINE_STIPPLE} functionnality.
\end{description}
...
\dd{gl2psPointSize} and \dd{gl2psLineSize} mimic the \dd{glPointSize} and
the \dd{glLineWidth} functions. These functions are necessary since the
point sizes and line widths are not saved in the OpenGL feedback buffer.
...
\begin{description}
\item[0.1] First distributed version.
\item[0.2] Added \dd{GL2PS_POLYGON_BOUNDARY} and \dd{GL2PS_BEST_ROOT}. Changed
arguments of \dd{gl2psBeginPage} and \dd{gl2psText}. Corrected some memory
allocation stuff. First version of this user's guide.
\item[0.21] Initialization fixes.
\item[0.3] Code cleaning. Added \dd{GL2PS_LINE_STIPPLE}.
\item[0.31] Better handling of erroneous primitives.
\item[0.32] Fixed memory leaks. 
\item[0.33] Added \dd{gl2psPointSize} and \dd{gl2psLineWidth}. Some code
cleaning to allow easier generation of other vector file formats than
postscript.
% Occlusion culling.
\end{description}
