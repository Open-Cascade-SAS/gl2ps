...
%
%  $Id: gl2ps.tex,v 1.22 2001-11-19 10:54:04 geuzaine Exp $
% 
...
\author{Christophe Geuzaine}
\date{Version 0.5, 19 November 2001}

\maketitle

%%tth: \section*{Download}
%%tth: The current distribution of GL2PS is available either as a
%%tth: \href{http://www.geuz.org/gl2ps/gl2ps-0.5.tar.gz}{tar.gz}
%%tth: or as a 
%%tth: \href{http://www.geuz.org/gl2ps/gl2ps-0.5.zip}{zip}
...
\begin{verbatim}
void gl2psBeginPage( char *title, char *producer, 
                     GLint format, GLint sort, GLint options, 
                     GLint colormode, GLint colorsize, GL2PSrgba *colortable, 
...
\item[\dd{format}] Specifies the output format, chosen among: \dd{GL2PS_PS},
\dd{GL2PS_TEX}. \dd{GL2PS_TEX} produces a \LaTeX\ file containing the text
parts of the images, and including an image having the same basename. This
image should be generated with the \dd{GL2PS_NO_TEXT} option.

\item[\dd{sort}] Specifies the sorting algorithm, chosen among:
\dd{GL2PS_NO_SORT}, \dd{GL2PS_SIMPLE_SORT}, \dd{GL2PS_BSP_SORT}.

\item[\dd{options}] Sets global plot options, chosen among: \dd{GL2PS_NONE},
\dd{GL2PS_DRAW_BACKGROUND}, \dd{GL2PS_SIMPLE_LINE_OFFSET},
\dd{GL2PS_SILENT}, \dd{GL2PS_BEST_ROOT}, \dd{GL2PS_NO_TEXT}. Multiple
options are combined with the bitwise inclusive OR symbol,~\dd{|}.
...
\item[\dd{filename}] Specifies name of the stream to which data is
printed. This argument is optional.
\end{description}
...
\item[\dd{GL2PS_NO_TEXT}] Suppress all text strings from output. Useful to
produce the image part of a \TeX\ output.
...
%file. This is not functional yet.
\end{description}
...
while( state == GL2PS_OVERFLOW ){ 
  buffsize += 1024*1024;
  gl2psBeginPage ( "MyTitle", "MySoftware", GL2PS_PS, GL2PS_BSP_SORT,
...
\item[0.5] (Nov 19, 2001) New format argument in \dd{gl2psBeginPage}. Better
PostScript handling of smooth shaded primitives. New options for LaTeX
output.
\end{description}
