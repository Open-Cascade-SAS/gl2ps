%  $Id: gl2ps.tex,v 1.117 2003-09-17 07:18:50 geuzaine Exp $
%
...
%%tth: \par
%%tth: To be notified when a new version of GL2PS is released,
%%tth: just subscribe to the
%%tth: \href{http://www.geuz.org/mailman/listinfo/gl2ps-announce/}
%%tth: {gl2ps-announce} mailing list!

\tableofcontents

\section{Introduction}

% GL2PS provides easy and high quality PostScript output for any OpenGL
% application. 

GL2PS is a library for creating high quality vector output (primarily
PostScript and PDF) from any OpenGL application. The main difference between
GL2PS and other similar libraries (see section~\ref{sec:links}) is the use
of sorting algorithms capable of handling intersecting and stretched
...
\item[\dd{GL2PS_TEX}] The output will be a \LaTeX\ file containing only the
  ...
  than PostScript or PDF. For example, you may export a bitmap image in JPEG
...
%\item[\dd{GL2PS_DEFLATE}] The output stream is compressed (currently only
%  available with the PDF format).
\end{description}
...
\item[\dd{nr}, \dd{ng}, \dd{nb}] (for PostScript output only) Controls the
  number flat-shaded (sub-)triangles used to approximate a smooth-shaded
  triangle when the \dd{shfill} operator is not supported by the system, or
  when the \dd{GL2PS_NO_PS3_SHADING} option is set. The arguments \dd{nr},
  \dd{ng} and \dd{nb} specify the number of values used for interpolating
  the full range of red, green and blue color components; that is, a
  triangle is recursively subdivided until the color difference between two
  of its vertices is smaller that $1/\mathtt{nr}$ for the red component,
  $1/\mathtt{ng}$ for the green component and $1/\mathtt{nb}$ for the blue
  component. If the arguments are set to zero, default values are used.
...
output the file directly in PDF format, though, replacing \dd{GL2PS_EPS}
with \dd{GL2PS_PDF} in the \dd{gl2psBeginPage()} function call above.
...
\item[1.0.0] (??, 2003) Native PDF support contributed by Micha Bieber.
\end{description}
