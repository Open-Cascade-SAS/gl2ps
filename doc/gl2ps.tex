...
%
%  $Id: gl2ps.tex,v 1.74 2003-03-05 18:29:33 geuzaine Exp $
% 
...
%%tth: \href{http://www.geuz.org/gl2ps/src/gl2ps-0.8.tar.gz}{tar.gz}
%%tth: or as a 
%%tth: \href{http://www.geuz.org/gl2ps/src/gl2ps-0.8.zip}{zip}
...
\begin{verbatim}
GLboolean gl2psBeginPage( const char *title, const char *producer,
...
arguments given to \dd{gl2psBeginPage} determine the way primitives are
handled:
...
PostScript files to PDF files; see below).
...
\item[\dd{nr}, \dd{ng}, \dd{nb}] Controls the number flat-shaded
...
\begin{verbatim}
GLboolean gl2psText( const char *string, const char *fontname,
                     GLint fontsize )
\end{verbatim}
...
\subsection{\texttt{gl2psPixmap}}
\label{sec:gl2psPixmap}

\subsubsection{Specification}

\begin{verbatim}
GLboolean gl2psPixmap( GLfloat x, GLfloat y, GLsizei w, GLsizei h,
                       GLfloat* image, GLboolean free )
\end{verbatim}

\subsubsection{Description and arguments}

\dd{gl2psPixmap} permits to include bitmap images in the PostScript
output. The image is inserted at the current raster position (set by one of
the \dd{glRasterPos} OpenGL commands). Beware that the image will be sorted
according to the position of the TODO only. The arguments are:

\begin{description}
\item[\dd{x}] Specifies ... TODO
...
\item[\dd{image}] Specifies ...

\item[\dd{free}] Specifies ...
\end{description}
...
\begin{verbatim}
GLboolean gl2psEnable( GLint mode )
\end{verbatim}

\begin{verbatim}
GLboolean gl2psDisable( GLint mode )
\end{verbatim}
...
\begin{verbatim}
GLboolean gl2psPointSize( GLfloat value )
\end{verbatim}

\begin{verbatim}
GLboolean gl2psLineWidth( GLfloat value )
\end{verbatim}
...
\begin{verbatim}
GLboolean gl2psBeginViewport ( void )
\end{verbatim}
...
\dd{gl2psBeginViewport} and \dd{gl2psEndViewport} TODO ...
...
\item
GL2PS does not support textures, volumic rendering, fog effects, etc.
\end{itemize}
...
the feedback buffer parser; \email{Bruce Naylor}{naylor@comp-graphics.com}
for BSP tree and occlusion culling hints; \email{Marc
Um{\'e}}{marc.ume@digitalgraphics.be} for the original list code;
\email{Jean-Fran\c{c}ois Remacle}{remacle@scorec.rpi.edu} for plane equation
fixes; \email{Bart Kaptein}{B.L.Kaptein@lumc.nl} for memory leak fixes;
...
triangle code; \email{Shane Hill}{Shane.Hill@dsto.defence.gov.au} for the
landscape option implementation; \email{Romain Boman}{r_boman@yahoo.fr} for
the Windows dll generation; \email{Rouben Rostamian}{rostamian@umbc.edu} for
various bug fixes; \email{Diego Santa Cruz}{Diego.SantaCruz@epfl.ch} for the
new optimized shaded triangle code and the \dd{shfill} management;
...
improvement of \dd{GL2PS_BEST_ROOT}; \email{Guy
Barrand}{barrand@lal.in2p3.fr} for \dd{gl2psPixmap} and the new viewport
managment.
...
\item[0.8] (Mar ??, 2003) API change: \dd{gl2psNumShadeColors} has been
removed and the color subdivision parameters \dd{nr}, \dd{ng} and \dd{nb}
are now given as arguments to \dd{gl2psBeginPage} (see
section~\ref{sec:gl2psBeginPage}); new \dd{gl2psPixmap} interface to produce
mixed mode PostScript/Bitmap output; new \dd{gl2psBeginViewport} and
\dd{gl2psEndViewport} interface to handle multiple OpenGL viewports; fixed
small bug in occlusion culling code.
\end{description}
